    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for row in hold_events:
            writer.writerow([
                ...
                int(row["cp2"]),
                int(row["cp_delta"]),
                "" if math.isnan(row["dt_us"]) else f"{row['dt_us']:.3f}",
                "" if math.isnan(row["freq_khz"]) else f"{row['freq_khz']:.3f}",
            ])
...
def write_numeric_markdown(path: Path, dlf_events: list[dict[str, float]], hold_events: list[dict[str, float]]) -> None:
    def dlf_row(row: dict[str, float]) -> list[str]:
        return [
            str(int(row["event"])),
            f"{row['sample_t_us']:.3f}",
            str(int(row["dd1"])),
            ...
            str(int(row["diff"])),
            str(int(row["diff_out"])),
            str(int(row["d_code"])),
            f"{row['oref']:.6f}",
        ]

    def hold_row(row: dict[str, float]) -> list[str]:
        return [
            str(int(row["event"])),
            f"{row['t_us']:.3f}",
            str(int(row["cp1"])),
